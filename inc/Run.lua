...
IRAQ = Token:match("(%d+)")
redis:set(IRAQ..":VERSION",2.9)
redis:set(IRAQ..":SUDO_ID:",SUDO_USER)
redis:set(IRAQ..":DataCenter:",'German')
redis:set(IRAQ..":UserNameBot:",BOT_User)
redis:set(IRAQ..":NameBot:",BOT_NAME)
redis:hset(IRAQ..'username:'..SUDO_USER,'username','@'..GetUser.result.username:gsub('_',[[\_]]))
redis:set("TH3IRAQ_INSTALL","Yes")
info = {}
...
IRAQ = Token:match("(%d+)")
our_id = tonumber(IRAQ)
ApiToken = "https://api.telegram.org/bot"..Token
Bot_User = redis:get(IRAQ..":UserNameBot:")
SUDO_ID = tonumber(redis:get(IRAQ..":SUDO_ID:"))
SUDO_USER = redis:hgetall(IRAQ..'username:'..SUDO_ID).username
version = redis:get(IRAQ..":VERSION")
DataCenter = redis:get(IRAQ..":DataCenter:")
...
..'¦ BOT__INFO: \27[1;34m'.. Bot_User..'\27[0;36m » ('..IRAQ..')\027[0;32m\n'
...
if msg.type == "pv" and redis:get(IRAQ..':mute_pv:'..msg.chat_id_) then
print('\27[1;31m is_MUTE_BY_FLOOD\27[0m')
return false 
end

if redis:get(IRAQ..'sender:'..msg.sender_user_id_..':flood') then
...
if redis:get(IRAQ..'group:add'..msg.chat_id_) then 
...
msg.Rank = 1
elseif redis:sismember(IRAQ..':SUDO_BOT:',msg.sender_user_id_) then 
msg.TheRankCmd = 'المطور 👨🏽‍💻'
msg.TheRank = 'مطور ثانوي  👨🏽‍💻'
msg.Rank = 2
elseif msg.GroupActive and redis:sismember(IRAQ..':KARA_BOT:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المنشىء الاساسي👷🏽'
msg.TheRank = 'المنشىء الاساسي👷🏽 '
msg.Rank = 3
elseif msg.GroupActive and redis:sismember(IRAQ..':MONSHA_BOT:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المنشىء 👷🏽'
msg.TheRank = 'المنشىء 👷🏽'
msg.Rank = 4
elseif msg.GroupActive and redis:sismember(IRAQ..'owners:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المدير 👨🏼‍⚕️' 
msg.TheRank = 'مدير البوت 👨🏼‍⚕️' 
msg.Rank = 5
elseif msg.GroupActive and redis:sismember(IRAQ..'admins:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'الادمن 👨🏼‍🎓'
msg.TheRank = 'ادمن في البوت 👨🏼‍🎓'
msg.Rank = 6
elseif msg.GroupActive and redis:sismember(IRAQ..'whitelist:'..msg.chat_id_,msg.sender_user_id_) then 
...
local lock_bots = redis:get(IRAQ..'lock_bots'..msg.chat_id_)
ZISBOT = false
...
if msg.GroupActive and ZISBOT and redis:get(IRAQ..'lock_bots_by_kick'..msg.chat_id_) then
kick_user(msg.sender_user_id_, msg.chat_id_)
end
if msg.content_.members_[0].id_ == our_id and redis:get(IRAQ..':WELCOME_BOT') then
SUDO_USER = redis:hgetall(IRAQ..'username:'..SUDO_ID).username
sendPhoto(msg.chat_id_,msg.id_,redis:get(IRAQ..':WELCOME_BOT'),[[🙋🏽‍♂╿ مـرحبآ آنآ بوت آسـمـي ]]..redis:get(IRAQ..':NameBot:')..[[ ⚜
...
if msg.GroupActive and redis:get(IRAQ..'mute_tgservice'..msg.chat_id_) then
Del_msg(msg.chat_id_,msg.id_)
return false 
end
...
if msg.Admin then redis:srem(IRAQ..'is_silent_users:'..msg.chat_id_,msg.sender_user_id_) return end
...
if msg.Admin then redis:srem(IRAQ..'banned:'..msg.chat_id_,msg.sender_user_id_) return end
...
if redis:get(IRAQ..'mute_text'..msg.chat_id_) then --قفل الدردشه
...
if ScriptFile and ScriptFile.IRAQ then 
if msg.text and ScriptFile.iIRAQ then
for k, IRAQ in pairs(ScriptFile.IRAQ) do
local SearchText = msg.text:match(IRAQ)
if SearchText then
if not CheckFlood(msg) then
return false 
end

local GetMsg = ScriptFile.iIRAQ(msg,{msg.text:match(IRAQ)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",IRAQ.." | Plugin is: \27[1;32mScript.lua\27[0m")
...
if ScriptFile.dIRAQ then
if ScriptFile.dIRAQ(msg) == false then
return false
end
...
if Plug.IRAQ then 
if msg.text and Plug.iIRAQ then
for k, IRAQ in pairs(Plug.IRAQ) do
local SearchText = msg.text:match(IRAQ)
if SearchText then
if not CheckFlood(msg) then
return false
...
Mohammad = msg.text
if Mohammad then
if redis:sismember(IRAQ..'CmDlist:'..msg.chat_id_,Mohammad) then
mmdi = redis:hget(IRAQ..'CmD:'..msg.chat_id_,Mohammad)
msg.text = Mohammad:gsub(Mohammad,mmdi)
end
end
local GetMsg = Plug.iIRAQ(msg,{msg.text:match(IRAQ)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",IRAQ.." | Plugin is: \27[1;32m"..name.."\27[0m")
sendMsg(msg.chat_id_,msg.id_,GetMsg)
end 
return false
end
...
print("The File "..name.." Not Runing in The Source IRAQ")
end 

end
else
print("The File Script.lua Not Runing in The Source IRAQ")
...
function tdcli_update_callback(data)
	...
	redis:srem(IRAQ..'users',msg.sender_user_id_)
	end
	...
	download_file('https://raw.githubusercontent.com/IRAQV1/IRAQ/master/inc/Run.lua','./inc/Run.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'40% - |█████       |')
	download_file('https://raw.githubusercontent.com/IRAQV1/IRAQ/master/inc/locks.lua','./inc/locks.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'60% - |███████     |')
	download_file('https://raw.githubusercontent.com/IRAQV1/IRAQ/master/inc/Script.lua','./inc/Script.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'80% - |█████████   |')
	download_file('https://raw.githubusercontent.com/IRAQV1/IRAQ/master/inc/functions.lua','./inc/functions.lua')
	download_file('https://raw.githubusercontent.com/IRAQV1/IRAQ/master/plugins/zhrfa.lua','./plugins/zhrfa.lua')
	download_file('https://raw.githubusercontent.com/IRAQV1/IRAQ/master/plugins/games.lua','./plugins/games.lua')
...
msg.text = Mohammad:gsub(Mohammad,mmdi)
	...
	mmd = redis:get(IRAQ..":SUDO_ID:")
redis:hset(IRAQ..'username:'..tonumber(mmd),'username',msg.text)
	...
	redis:set(IRAQ..":SUDO_ID:",msg.text)
...
download(stk,32)
	...
	if redis:get(IRAQ..'group:add'..msg.chat_id_) and (msg.sender_user_id_ == SUDO_ID or redis:sismember(IRAQ..':SUDO_BOT:',msg.sender_user_id_) or redis:sismember(IRAQ..':MONSHA_BOT:'..msg.chat_id_,msg.sender_user_id_) or redis:sismember(IRAQ..'owners:'..msg.chat_id_,msg.sender_user_id_) or redis:sismember(IRAQ..'admins:'..msg.chat_id_,msg.sender_user_id_)) then 
	msg.Admin = true
	end
	local lock_bots = redis:get(IRAQ..'lock_bots'..msg.chat_id_)
	ISBOT = false
	...
	if ZISBOT and redis:get(IRAQ..'lock_bots_by_kick'..msg.chat_id_) then
	kick_user(msg.sender_user_id_, msg.chat_id_)
	end
...
if redis:get(IRAQ..'lock:join:'..msg.chat_id_) then
kick_user(msg.sender_user_id_,msg.chat_id_)
	...
	if redis:get(IRAQ..'group:add'..data.chat_.id_) then
	redis:set(IRAQ..'group:name'..data.chat_.id_,data.chat_.title_)
	end
	...
	if redis:get(IRAQ..'group:add-100'..data.channel_.id_) then
	local linkGroup = (redis:get(IRAQ..'linkGroup-100'..data.channel_.id_) or "")
	local NameGroup = (redis:get(IRAQ..'group:name-100'..data.channel_.id_) or "")
	...
	redis:mset(
	IRAQ..'group:name'..k,IDS.Title,
	IRAQ..'num_msg_IRAQ'..k,5,
	IRAQ..'group:add'..k,true,
	IRAQ..'lock_link'..k,true,
	IRAQ..'lock_spam'..k,true,
	IRAQ..'lock_webpage'..k,true,
	IRAQ..'lock_markdown'..k,true,
	IRAQ..'lock_flood'..k,true,
	IRAQ..'lock_bots'..k,true,
	IRAQ..'mute_forward'..k,true,
	IRAQ..'mute_contact'..k,true,
	IRAQ..'mute_document'..k,true,
	IRAQ..'mute_inline'..k,true,
	IRAQ..'lock_username'..k,true,
	IRAQ..'replay'..k,true
	)
	redis:sadd(IRAQ..'group:ids',k) 

	if IDS.Admins then
	for user,ID in pairs(IDS.Admins) do
	redis:hset(IRAQ..'username:'..ID,'username',user)
	redis:sadd(IRAQ..'admins:'..k,ID)
	end
	end
	if IDS.Creator then
	for user,ID in pairs(IDS.Creator) do
	redis:hset(IRAQ..'username:'..ID,'username',user)
	redis:sadd(IRAQ..':MONSHA_BOT:'..k,ID)
	end
	end
	if IDS.Owner then
	for user,ID in pairs(IDS.Owner) do
	redis:hset(IRAQ..'username:'..ID,'username',user)
	redis:sadd(IRAQ..'owners:'..k,ID)
	end
	end
	end
	io.popen("rm -fr ../.telegram-cli/data/document/*")
	sendMsg(Uploaded_Groups_CH,Uploaded_Groups_MS,'📦*¦* تم رفع آلنسـخهہ‏‏ آلآحتيآطـيهہ\n⚖️*¦* حآليآ عدد مـجمـوعآتگ هہ‏‏يهہ‏‏ *'..redis:scard(IRAQ..'group:ids')..'* 🌿\n✓')
	end
	elseif data.ID == "UpdateUser" then  
	if data.user_.type_.ID == "UserTypeDeleted" then
	print("¦ userTypeDeleted")
	redis:srem(IRAQ..'users',data.user_.id_)
	elseif data.user_.type_.ID == "UserTypeGeneral" then
	local CheckUser = redis:hgetall(IRAQ..'username:'..data.user_.id_).username
	...
	redis:hset(IRAQ..'username:'..data.user_.id_,'username',USERNAME)
	end 
	end
